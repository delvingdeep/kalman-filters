...
** This is a part of "Build MCL in C++" lab in term 2 of Udacity RoboND
**
*/
...
double landmarks[8][2] = { { 20.0, 20.0 }, { 20.0, 80.0 }, { 20.0, 50.0 }, { 50.0, 20.0 }, 
                           { 50.0, 80.0 }, { 80.0, 80.0 }, { 80.0, 20.0 }, { 80.0, 50.0 } };    // Landmark in the form of a 2D array

double world_size = 100.0;  // Map size in meters
...
Robot::Robot() {
    // Constructor
    Robot::x = gen_real_random() * world_size; // robot's x coordinate
    Robot::y = gen_real_random() * world_size; // robot's y coordinate
    Robot::orient = gen_real_random() * 2.0 * M_PI; // robot's orientation

    Robot::forward_noise = 0.0; //noise of the forward movement
    Robot::turn_noise = 0.0; //noise of the turn
    Robot::sense_noise = 0.0; //noise of the sensing
}
...
void Robot::set_noise(double new_forward_noise, double new_turn_noise, double new_sense_noise) {
    ...
    Robot::forward_noise = new_forward_noise;
    Robot::turn_noise = new_turn_noise;
    Robot::sense_noise = new_sense_noise;
}
...
vector<double> Robot::sense() {
    ...
    for (int i = 0; i < sizeof(landmarks) / sizeof(landmarks[0]); i++) {
        dist = sqrt(pow((Robot::x - landmarks[i][0]), 2) + pow((Robot::y - landmarks[i][1]), 2));
        dist += gen_gauss_random(0.0, Robot::sense_noise);
        z[i] = dist;
    }
    return z;
}
